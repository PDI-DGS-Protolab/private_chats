...
// Our fix involves replicating a bunch of files in order to                                               // 2
//                                                                                                         // 3
if (process.version !== 'v0.10.22' && process.version !== 'v0.10.23') {                                    // 4
  if (!process.env.DISABLE_WEBSOCKETS) {                                                                   // 5
    console.error("This version of Meteor contains a patch for a bug in Node v0.10.");                     // 6
    console.error("The patch is against only versions 0.10.22 and 0.10.23.");                              // 7
    console.error("You are using version " + process.version + " instead, so we cannot apply the patch."); // 8
    console.error("To mitigate the most common effect of the bug, websockets will be disabled.");          // 9
    console.error("To enable websockets, use Node v0.10.22 or .23, or upgrade to a later version of Meteor (if available).");
    process.env.DISABLE_WEBSOCKETS = 't';                                                                  // 11
  }                                                                                                        // 12
} else {                                                                                                   // 13
  // This code is all copied from Node's lib/_stream_writable.js, git tag                                  // 14
  // v0.10.22, with one change (see "BUGFIX").                                                             // 15
  var Writable = Npm.require('_stream_writable');                                                          // 16
  var Duplex = Npm.require('_stream_duplex');                                                              // 17
                                                                                                           // 18
  Writable.prototype.write = function(chunk, encoding, cb) {                                               // 19
    var state = this._writableState;                                                                       // 20
    var ret = false;                                                                                       // 21
                                                                                                           // 22
    if (typeof encoding === 'function') {                                                                  // 23
      cb = encoding;                                                                                       // 24
      encoding = null;                                                                                     // 25
    }                                                                                                      // 26
                                                                                                           // 27
    if (Buffer.isBuffer(chunk))                                                                            // 28
      encoding = 'buffer';                                                                                 // 29
    else if (!encoding)                                                                                    // 30
      encoding = state.defaultEncoding;                                                                    // 31
                                                                                                           // 32
    if (typeof cb !== 'function')                                                                          // 33
      cb = function() {};                                                                                  // 34
                                                                                                           // 35
    if (state.ended)                                                                                       // 36
      writeAfterEnd(this, state, cb);                                                                      // 37
    else if (validChunk(this, state, chunk, cb))                                                           // 38
      ret = writeOrBuffer(this, state, chunk, encoding, cb);                                               // 39
                                                                                                           // 40
    return ret;                                                                                            // 41
  };                                                                                                       // 42
                                                                                                           // 43
  // Duplex doesn't directly inherit from Writable: it copies over this function                           // 44
  // explicitly. So we have to do it too.                                                                  // 45
  Duplex.prototype.write = Writable.prototype.write;                                                       // 46
                                                                                                           // 47
  function writeAfterEnd(stream, state, cb) {                                                              // 48
    var er = new Error('write after end');                                                                 // 49
    // TODO: defer error events consistently everywhere, not just the cb                                   // 50
    stream.emit('error', er);                                                                              // 51
    process.nextTick(function() {                                                                          // 52
      cb(er);                                                                                              // 53
    });                                                                                                    // 54
  }                                                                                                        // 55
                                                                                                           // 56
  function validChunk(stream, state, chunk, cb) {                                                          // 57
    var valid = true;                                                                                      // 58
    if (!Buffer.isBuffer(chunk) &&                                                                         // 59
        'string' !== typeof chunk &&                                                                       // 60
        chunk !== null &&                                                                                  // 61
        chunk !== undefined &&                                                                             // 62
        !state.objectMode) {                                                                               // 63
      var er = new TypeError('Invalid non-string/buffer chunk');                                           // 64
      stream.emit('error', er);                                                                            // 65
      process.nextTick(function() {                                                                        // 66
        cb(er);                                                                                            // 67
      });                                                                                                  // 68
      valid = false;                                                                                       // 69
    }                                                                                                      // 70
    return valid;                                                                                          // 71
                                                                                                           ...
                                                                                                           // 73
  function writeOrBuffer(stream, state, chunk, encoding, cb) {                                             // 74
    chunk = decodeChunk(state, chunk, encoding);                                                           // 75
    if (Buffer.isBuffer(chunk))                                                                            // 76
      encoding = 'buffer';                                                                                 // 77
    var len = state.objectMode ? 1 : chunk.length;                                                         // 78
                                                                                                           // 79
    state.length += len;                                                                                   // 80
                                                                                                           // 81
    var ret = state.length < state.highWaterMark;                                                          // 82
    // This next line is the BUGFIX:                                                                       // 83
    state.needDrain = state.needDrain || !ret;                                                             // 84
                                                                                                           // 85
    if (state.writing)                                                                                     // 86
      state.buffer.push(new WriteReq(chunk, encoding, cb));                                                // 87
    else                                                                                                   // 88
      doWrite(stream, state, len, chunk, encoding, cb);                                                    // 89
                                                                                                           // 90
    return ret;                                                                                            // 91
  }                                                                                                        // 92
                                                                                                           // 93
  function decodeChunk(state, chunk, encoding) {                                                           // 94
    if (!state.objectMode &&                                                                               // 95
        state.decodeStrings !== false &&                                                                   // 96
        typeof chunk === 'string') {                                                                       // 97
      chunk = new Buffer(chunk, encoding);                                                                 // 98
    }                                                                                                      // 99
    return chunk;                                                                                          // 100
  }                                                                                                        // 101
                                                                                                           // 102
  function WriteReq(chunk, encoding, cb) {                                                                 // 103
    this.chunk = chunk;                                                                                    // 104
    this.encoding = encoding;                                                                              // 105
    this.callback = cb;                                                                                    // 106
  }                                                                                                        // 107
                                                                                                           // 108
  function doWrite(stream, state, len, chunk, encoding, cb) {                                              // 109
    state.writelen = len;                                                                                  // 110
    state.writecb = cb;                                                                                    // 111
    state.writing = true;                                                                                  // 112
    state.sync = true;                                                                                     // 113
    stream._write(chunk, encoding, state.onwrite);                                                         // 114
    state.sync = false;                                                                                    // 115
  }                                                                                                        // 116
}                                                                                                          // 117
                                                                                                           // 118
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
